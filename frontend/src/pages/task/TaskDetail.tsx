...
export const TaskDetail: React.FC = () => {
  ...
  const progress = subtasks.length > 0 ? Math.round((completedSubtasks / subtasks.length) * 100) : (task.status == TaskStatus.DONE ? 100 : 0);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  {subtasks.length > 0 ? (
                    subtasks.map((subtask) => (
                      <div 
                        ...
                        onClick={() => navigate(`/tasks/${subtask.id}`, { state: { task: subtask } })}
                      >
                        <div className={`w-2 h-2 rounded-full ${subtask.status === TaskStatus.DONE ? 'bg-green-500' : 'bg-gray-300'}`} />
                        <span className={`flex-1 font-medium ${subtask.status === TaskStatus.DONE ? 'line-through text-gray-400' : 'text-gray-700'}`}>
                          {subtask.title}
                        </span>
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${priorityColors[subtask.priority]}`}>
                          {subtask.priority}
                        </span>
                      </div>
                    ))
                  ) : (
                    <p className="text-gray-400 text-sm italic">No subtasks added to this task.</p>
                  )}
                  <button className="w-full py-3 mt-2 border-2 border-dashed border-gray-200 rounded-xl text-gray-400 hover:border-blue-300 hover:text-blue-500 transition-all text-sm font-semibold">
                    + Add New Subtask
                  </button>
                </div>
              </div>
          ...
          <aside className="lg:col-span-1 space-y-6">
            <div className="bg-white rounded-2xl shadow-md p-6 border border-gray-100">
              ...
              <div className="space-y-6">
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase block mb-2">Current Status</label>
                ...
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase block mb-2">Priority Level</label>
                ...
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase block mb-2">Created On</label>
                ...
                {task.parentId && (
                  <div className="pt-4 border-t border-gray-50">
                    <label className="text-[10px] font-bold text-gray-400 uppercase block mb-2">Parent Task</label>
                    <button 
